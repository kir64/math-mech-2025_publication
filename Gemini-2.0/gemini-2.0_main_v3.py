...
def test_validate_get_pet_find_by_status_status_codes():
    ...
    validate_status_codes("/pet/findByStatus", "GET", expected, params={"status": "invalid"}) # Некорректный статус
...
def test_validate_get_pet_find_by_tags_status_codes():
    ...
    validate_status_codes("/pet/findByTags", "GET", expected, params={"tags": "invalid tag"}) # Некорректный тег

def test_validate_get_pet_by_id_status_codes():
    expected = get_expected_status_codes("/pet/{petId}", "GET")
    validate_status_codes("/pet/123", "GET", expected) # Валидный формат ID (теоретически)
